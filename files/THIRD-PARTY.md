...
| `libasound2-plugins` and the parts of its dependency chain that the image needs: `libavcodec61`, `libavutil59`, `libswresample5`, `libsamplerate0`, `libspeexdsp1`, `libpulse0`, `libjack-jackd2-0` | `1.2.12-2+b1` (ffmpeg 7.1.5, libsamplerate 0.2.2, libspeexdsp 1.2.1) | LGPL-2.1+ (libsamplerate0: **BSD-2-Clause**, libspeexdsp1: **BSD-3-Clause**) | Supplies the sample-rate converter used for 44.1 kHz material (`rate_converter "samplerate"`, i.e. libsamplerate through the ALSA `plug` plugin). `licenses/LGPL-2.1.txt` is shipped; the BSD texts and the rest stay in each `/usr/share/doc/<pkg>/copyright` |
| `libsoxr0` (via `shairport-sync`, linked as `libsoxr.so.0`) | `0.1.3-4+b2` | **LGPL-2.1+** | AirPlay resampling: shairport-sync is built `--with-soxr` and uses it for the continuous ~1000 ppm clock-drift correction (`interpolation = "soxr"` in `/etc/shairport-sync.conf`). `shairport-sync` **Depends: libsoxr0**, so the library is in the image; the same `licenses/LGPL-2.1.txt` covers it, and the package's own copyright stays in `/usr/share/doc/libsoxr0/copyright` |
